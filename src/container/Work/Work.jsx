...
const Work=()=> {
  ...
  setWorks]=useState([]);
  const [filterWork,
  setFilterWork]=useState([]);
  const [activeFilter,
  setActiveFilter]=useState('All');

  const [animateCard,
  setAnimateCard]=useState( {
      y: 0, opacity: 1
    }

  );
  const [visibleItems,
  setVisibleItems]=useState(6);
  const [showLoadMore,
  setShowLoadMore]=useState(true);

  // Modal state
  const [selectedWork,
  setSelectedWork]=useState(null);
  const [isModalOpen,
  setIsModalOpen]=useState(false);

  // Open modal to show work detail
  ...
  const currentModalIndex=selectedWork ? filterWork.findIndex(w=> w.id===selectedWork.id) : -1;
  const hasPrevious=currentModalIndex>0;
  const hasNext=currentModalIndex < filterWork.length - 1;

  const goToPrevious=useCallback(()=> {
      if (hasPrevious) {
        setSelectedWork(filterWork[currentModalIndex - 1]);
      }
    }

    , [hasPrevious, currentModalIndex, filterWork]);
  ...
  const goToNext=useCallback(()=> {
    ...
    , [hasNext, currentModalIndex, filterWork]);
  ...
  const stripHtmlAndMarkdown=(text)=> {
    ...
    .replace(/\[([^\]]+)\]\([^)]+\)/g,
    '$1') // Convert markdown links to text
  ...
  const isCICDTag=(tag)=> {
    const cicdKeywords=['github actions',
    'github-actions',
    'gitlab ci',
    'gitlab-ci',
    'jenkins',
  ...
  const handleWorkFilter=(item)=> {
    ...
    setTimeout(()=> {
        ...
        if (item==='All') {
          ...
          filtered=works.filter((work)=> work.tags && work.tags.some(tag=> isDatabaseTag(tag)));
        }

        else if (item==='Mobile') {
          // Smart filtering for mobile category (Android, iOS, React Native, Flutter, etc.)
          filtered=works.filter((work)=> work.tags && work.tags.some(tag=> isMobileTag(tag)));
        }

        else if (item==='Desktop') {
          // Smart filtering for desktop category (Windows, Mac, Linux, Electron, etc.)
          filtered=works.filter((work)=> work.tags && work.tags.some(tag=> isDesktopTag(tag)));
        }

        else if (item==='CI/CD') {
          // Smart filtering for CI/CD category
          filtered=works.filter((work)=> work.tags && work.tags.some(tag=> isCICDTag(tag)));
        }

        else if (item==='Docker') {
          // Smart filtering for Docker/Container category
          filtered=works.filter((work)=> work.tags && work.tags.some(tag=> isDockerTag(tag)));
        }
  ...
  return (<> <h2 className="head-text">My Creative <span>Portfolio</span> Section</h2> <div className="app__work-filter"> {
      ['All', 'React', 'TypeScript', 'Next.js', 'Python', 'Mobile', 'Desktop', 'Documentation', 'Database', 'CI/CD', 'Docker'].map((item, index)=> (<div key= {
            index
          }
      ...
      {
      filterWork.length===0 && (<div style= {
            {
            textAlign: 'center', padding: '2rem', color: '#666'
          }
        }

        > <p>No projects to display. Loading...</p> </div>)
    }
    ...
    > {
      filterWork.slice(0, visibleItems).map((work, index)=> (<div className="app__work-item app__flex"
          ...
          transition= {
              {
              duration: 0.25, ease: 'easeInOut', staggerChildren: 0.5
            }
          }
          ...
          > <AiFillGithub /> </motion.div> </a> </motion.div> </div> <div className="app__work-content app__flex"> <h4 className="bold-text"> {
            work.title
          }

          </h4> <p className="p-text app__work-description"style= {
              {
              marginTop: 10
            }
          }
          ...
          > {
            ...
            work.tags && work.tags.length > 0 && (<div className="app__work-tags-container"> {
                (Array.isArray(work.tags) ? work.tags.slice(0, 3) : [work.tags]).map((tag, tagIndex)=> (<span key= {
                      tagIndex
                    }
                ...
                {
                work.tags.length > 3 && (<span className="app__work-tag-more">+ {
                    work.tags.length - 3
                  }
      ...
      filterWork.length > 6 && (<div className="app__work-loadmore"> <button onClick= {
          handleLoadMore
        }
      ...
      >
        ...
        > Show Less </button> <button onClick= {
          ()=> {
            ...
            window.scrollTo( {
                top: document.getElementById('work')?.offsetTop || 0, behavior: 'smooth'
              }
        ...
        disabled= {
          visibleItems===6 && activeFilter==='All'
        }
      ...
      {
      works.length > 0 && (<div className="app__work-view-all"> <ViewAllButton to="/works"
