...
import { FiSearch, FiPlus, FiCheck, FiX, FiLoader, FiExternalLink, FiChevronDown } from 'react-icons/fi';
...
const PortfolioContentPicker = ({
    ...
    const currentTypeInfo = CONTENT_TYPES.find(t => t.key === contentType) || { label: contentType, icon: '📄' };
    ...
    const handleConfirm = () => {
        pendingSelections.forEach(item => {
            // Pass both the transformed item and the current content type
            onSelect(transformItemForResume(item, contentType), contentType);
        });
        onClose();
    };

    // Transform portfolio item to resume format
    const transformItemForResume = (item, type) => {
        switch (type) {
            case 'experiences':
                return {
                    id: Date.now(),
    ...
    const renderItem = (item) => {
        ...
        return (
            <div
                ...
                onClick={() => !isAlreadyAdded && toggleSelection(item)}
            >
                <div className="item-checkbox">
                    {isAlreadyAdded ? (
                        <FiCheck className="already-added" />
                    ) : isPending ? (
                        <FiCheck className="checked" />
                    ) : (
    ...
    const renderItemContent = (item) => {
        switch (contentType) {
            case 'experiences':
                return (
                    <>
                        <div className="item-title">{item.position || item.name}</div>
                        <div className="item-subtitle">{item.company}</div>
                        <div className="item-meta">
                            {formatPeriod(item.startDate, item.endDate, item.isCurrent)}
                            {item.location && ` • ${item.location}`}
                        </div>
                        {item.resumeSummary && <div className="item-summary">{item.resumeSummary}</div>}
                    </>
                );
            ...
            case 'education':
                return (
                    <>
                        <div className="item-title">{item.degree} in {item.field}</div>
            ...
            case 'awards':
                return (
                    <>
                        ...
                        <div className="item-subtitle">{item.issuer}</div>
                        <div className="item-meta">{item.year}</div>
                    </>
                );
                ...
                return (
                    <>
                        ...
                        <div className="item-meta">
                            ...
                            {item.projectLink && (
                                <a href={item.projectLink} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()}>
                                    <FiExternalLink />
                                </a>
                            )}
                        </div>
            ...
            case 'skills':
                return (
                    <>
                        ...
                        <div className="item-meta">
                            {item.proficiencyLevel && `Level: ${item.proficiencyLevel}/10`}
            ...
            case 'publications':
                return (
                    <>
                        <div className="item-title">{item.title}</div>
                        <div className="item-subtitle">{item.venue}</div>
                        <div className="item-meta">
                            {item.year}
            ...
            case 'leadership':
                return (
                    <>
                        <div className="item-title">{item.title}</div>
                        <div className="item-subtitle">{item.organization}</div>
                        <div className="item-meta">
                            {item.role}
                            {formatPeriod(item.startDate, item.endDate, item.isCurrent)}
                        </div>
            ...
            case 'references':
                return (
                    <>
                        <div className="item-title">{item.name}</div>
                        <div className="item-subtitle">{item.title} at {item.company}</div>
                        <div className="item-meta">
                            {item.relationship}
            ...
            case 'languages':
                return (
                    <>
                        ...
                        <div className="item-subtitle">{item.proficiency}</div>
                        <div className="item-meta">
            ...
            default:
                return (
                    <div className="item-title">{item.name || item.title || 'Unknown'}</div>
                );
        }
    };
    ...
    return (
        <div className="portfolio-content-picker-overlay" onClick={onClose}>
            <div className="portfolio-content-picker" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="header-title-row">
                        ...
                        {allowTypeSwitch ? (
                            <div className="type-switcher">
                                ...
                                >
                                    <span className="type-icon">{currentTypeInfo.icon}</span>
                                    <span className="type-label">{currentTypeInfo.label}</span>
                                    <FiChevronDown className={`dropdown-arrow ${showTypeDropdown ? 'open' : ''}`} />
                                </button>
                                {showTypeDropdown && (
                ...
                <div className="picker-content">
                    {loading ? (
                        ...
                        <div className="picker-empty">
                            <span>No {currentTypeInfo.label.toLowerCase()} found.</span>
                ...
                <div className="picker-footer">
                    <span className="selection-count">
                        {pendingSelections.length} item{pendingSelections.length !== 1 ? 's' : ''} selected
                    </span>
